...
class ControllerModulePavautosearch extends Controller {
	...
	public function index() {   
		$this->language->load('module/pavautosearch');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->redirect($this->url->link('module/pavautosearch', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$this->data['action'] = $this->url->link('module/pavautosearch', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['positions'] = array( 'call_by_search','mainmenu',
										  'slideshow',
										  'promotion',
										  'content_top',
										  'column_left',
		...
		if (isset($this->request->post['pavautosearch_module'])) {
			$this->data['modules'] = $this->request->post['pavautosearch_module'];
		} elseif ($this->config->get('pavautosearch_module')) { 
			$this->data['modules'] = $this->config->get('pavautosearch_module');
		}
		...
		$this->template = 'module/pavautosearch.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
