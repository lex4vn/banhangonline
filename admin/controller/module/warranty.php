...
class ControllerModuleWarranty extends Controller {
	...
	public function index() {   
		$this->load->language('module/warranty');
		...
		$image_avatar_w = array();
		
		if (isset($this->request->post['image_avatar_w'])) {
			$image_avatar_w = $this->request->post['image_avatar_w'];
		} elseif ($this->config->get('image_avatar_w')) { 
			$image_avatar_w = $this->config->get('image_avatar_w');
		}	
		
		$this->data['images'] = array();
		
		foreach ($image_avatar_w as $image_avatar_w) {
			if ($image_avatar_w['image'] && file_exists(DIR_IMAGE . $image_avatar_w['image'])) {
				$image = $image_avatar_w['image'];
			} else {
				$image = 'no_image.jpg';
			}			
			
			$this->data['images'][] = array(
				'position'                      => $image_avatar_w['position'],
				'name'	 			=> $image_avatar_w['name'],
				'phone'	 			=> $image_avatar_w['phone'],
				'email'	 			=> $image_avatar_w['email'],
				'yahoo'	 			=> $image_avatar_w['yahoo'],
				'skype'	 			=> $image_avatar_w['skype'],
				'image'                    => $image,
		...
		if (isset($this->request->post['warranty_module'])) {
			$this->data['modules'] = $this->request->post['warranty_module'];
		} elseif ($this->config->get('warranty_module')) { 
			$this->data['modules'] = $this->config->get('warranty_module');
		}					
	...
	private function validate() {
		...
		if (isset($this->request->post['warranty_module'])) {
			foreach ($this->request->post['warranty_module'] as $key => $value) {
