<div class="<?php echo $additional_class; ?> box search_block">
	...
	<div id="search<?php echo $module ?>" class="search">
	    <input type="text" value="" size="35" autocomplete="off" placeholder="<?php echo $this->language->get("text_search");?>" name="search">
	    <input type="submit" class="button button-search" value="<?php echo $this->language->get("text_submit_search");?>" name="submit_search">
	</div>
	...
	$(selector).find('input[name=\'search\']').autocomplete({
		...
		$(selector).find('input[name=\'search\']').data( "autocomplete" )._renderItem = function( ul, item ) {
			...
			if(show_price){
				...
				html += '<p style="font-size:0.9em;line-height:1.4em;margin:2px 0 0;"></p><div class="std"><?php echo $this->language->get("text_price");?>'+item.price;	
			}
